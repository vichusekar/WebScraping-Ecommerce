...
function Books() {
    ...
    let handlegetBooks = async () => {
        try {
            let res = await axios.get(`${process.env.REACT_APP_API_URL}/books`)
            if (res.status === 200) {
    ...
    return <>
        <Header />
        <div className='d-flex w-60% flex-wrap' style={{ justifyContent: 'space-around', marginLeft: '100px', size: '10px', height: '200px' }}>
            {
                data.map((e, i) => {

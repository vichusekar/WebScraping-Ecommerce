...
function Electronics() {
    ...
    let handlegetProducts = async () => {
        try {
            let res = await axios.get(`${process.env.REACT_APP_API_URL}/electronics`)
            if (res.status === 200) {
    ...
    return <>
        <Header />
        <div className='d-flex w-60% flex-wrap' style={{ justifyContent: 'space-around', marginLeft: '10px', size: '10px', height: '200px' }}>
            {
                data.map((e, i) => {
